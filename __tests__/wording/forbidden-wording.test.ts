...
const starterCourse = require('@/data/seed/starter_7d.json') as Record<string, unknown>;
...
function collectCatalogText(): string[] {
  ...
  // triggers
  const triggers = catalog.triggers as Array<{ label: string; description: string }>;
  ...
  // spendDelayCards
  const cards = catalog.spendDelayCards as Array<{ title: string; body: string }>;
 ...
 */
const SEXUAL_WORDING_PATTERN = /\b(sex(?:ual(?:ly)?|ually|ualize[sd]?)?|erotic(?:ally)?|porn(?:ographic)?|nude|naked(?:ly)?|nsfw)\b/i;

/**
 * Cure or treatment claims — the app is not medical/therapy and must not
 * claim to cure or treat any condition.
 */
const CURE_TREATMENT_PATTERN = /\b(cure[sd]?|curing|treatment|treating|treats|therapy|therapist|diagnos(?:e[sd]?|is|tic)?|prescri(?:be[sd]?|ption))\b/i;

/**
 * Perfect blocking claims — must not promise perfect or guaranteed prevention.
 */
const PERFECT_BLOCKING_PATTERN = /\b(perfect(?:ly)?|guaranteed?|100%)\s+(block|stop|prevent|eliminat|remov)[a-z]*/i;
 ...
 */
const DATING_COACHING_PATTERN = /\b(?:relationship|dating)\s+coach(?:ing|es|ed|er)?/i;

/**
 * Forced lockout — must not describe or promise forced blocking/lockout mechanisms.
 */
const FORCED_LOCKOUT_PATTERN = /\b(?:forced?|involuntary)\s+(?:lock(?:out|ed|s)?|block(?:ed|s)?|prevent(?:ed|s)?)/i;
...
describe('Pattern sanity checks', () => {
  ...
  it('PERFECT_BLOCKING_PATTERN catches "perfectly blocks"', () => {
    expect('perfectly blocks all urges'.match(PERFECT_BLOCKING_PATTERN)).not.toBeNull();
  });

  it('DATING_COACHING_PATTERN catches "dating coaching"', () => {
    expect('dating coaching service'.match(DATING_COACHING_PATTERN)).not.toBeNull();
  });

  it('FORCED_LOCKOUT_PATTERN catches "forced lockout"', () => {
    expect('forced lockout feature'.match(FORCED_LOCKOUT_PATTERN)).not.toBeNull();
  });
});
