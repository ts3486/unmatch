  ...
  useEffect(() => {
    if (prevStepRef.current === 'breathing' && flow.step === 'select_action') {
      analytics.track({
        ...
        props: { urge_kind: flow.urgeKind ?? 'check', duration_ms: BREATHING_DURATION_SECONDS * 1000 },
      });
    }
    ...
    return (
      <SelectUrgeStep
        onSelect={(kind: UrgeKind) => {
          analytics.track({ name: 'panic_started', props: { urge_kind: kind, from_screen: 'panic' } });
  ...
  if (flow.step === 'breathing') {
    return (
      <BreathingStep
        ...
        onSkip={() => {
          const elapsed = (BREATHING_DURATION_SECONDS - flow.breathingTimeLeft) * 1000;
          analytics.track({ name: 'panic_protocol_completed', props: { urge_kind: flow.urgeKind ?? 'check', duration_ms: elapsed } });
          flow.skipBreathing();
        }}
      />
    );
    ...
    return (
      <SelectActionStep
        ...
        onSelect={(actionId: string) => {
          analytics.track({ name: 'panic_action_selected', props: { urge_kind: flow.urgeKind ?? 'check', action_id: actionId } });
  ...
  if (flow.step === 'spend_delay') {
    return (
      <SpendDelayStep
        ...
        onResisted={() => {
          analytics.track({ name: 'panic_outcome_logged', props: { urge_kind: flow.urgeKind ?? 'spend', outcome: 'success', trigger_tag: null } });
          void flow.logOutcome('success');
        }}
        onSpentAnyway={() => {
          analytics.track({ name: 'panic_outcome_logged', props: { urge_kind: flow.urgeKind ?? 'spend', outcome: 'fail', trigger_tag: null } });
    ...
    return (
      <LogOutcomeStep
        ...
        onLogOutcome={async (outcome, triggerTag) => {
          analytics.track({ name: 'panic_outcome_logged', props: { urge_kind: flow.urgeKind ?? 'check', outcome, trigger_tag: triggerTag ?? null } });
...
function SelectUrgeStep({ urgeKinds, onSelect }: SelectUrgeStepProps): React.ReactElement {
  const catalog = getCatalog();
...
function BreathingStep({ timeLeft, totalDuration, onSkip }: BreathingStepProps): React.ReactElement {
  return (
    <View style={styles.stepContainer}>
      <ScrollView
        style={styles.stepScroll}
...
interface SelectActionStepProps {
  actions: Array<{ id: string; title: string; body: string; est_seconds: number; action_type: string }>;
...
function SelectActionStep({
  ...
  const sorted = [...filteredActions].sort((a, b) => a.est_seconds - b.est_seconds);
...
interface SpendDelayStepProps {
  spendDelayCards: Array<{ id: string; title: string; body: string; action_id: string }>;
  actions: Array<{ id: string; title: string; body: string; est_seconds: number; action_type: string }>;
...
interface LogOutcomeStepProps {
  ...
  onLogOutcome: (outcome: UrgeOutcome, triggerTag?: string | null) => Promise<void>;
}
            ...
            <View style={styles.chipRow}>
              {spendCategories.map((cat) => (
                <Chip
                  ...
                  textStyle={[
                    styles.triggerChipText,
                    selectedCategory === cat.id && styles.triggerChipTextSelected,
                  ]}
                  compact
                >
                  {cat.label}
        ...
        >
          ...
          >
            <MaterialCommunityIcons
              ...
              size={64}
              color={
                isSuccess ? colors.success : isOngoing ? colors.warning : colors.muted
              }
            />
          </View>
        </Animated.View>
...
function ActionCard({ title, estMinutes, onPress }: ActionCardProps): React.ReactElement {
  return (
