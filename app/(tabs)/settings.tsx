...
export default function SettingsScreen(): React.ReactElement {
  ...
  const handleLockToggle = useCallback(async (value: boolean): Promise<void> => {
    ...
    setIsUpdating(true);
    try {
      await updateUserProfile(db, userProfile.id, { lock_enabled: value ? 1 : 0 });
    ...
    >
      ...
      <View style={styles.listCard}>
        <List.Item
          ...
          titleStyle={styles.listTitle}
          descriptionStyle={styles.listDesc}
          onPress={() => { router.push('/settings/blocker-guide'); }}
          right={({ color }) => (
