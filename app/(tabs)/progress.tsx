...
import { getLocalDateString, getDaysBetween } from '@/src/utils/date';
      ...
      <Card.Content>
        ...
        <View style={styles.comparisonRow}>
          <View style={styles.comparisonCol}>
            ...
            >
              {daysDelta === 0 ? '— same' : `${deltaStr(daysDelta)} vs last week`}
            </Text>
          </View>
          ...
          <View style={styles.comparisonCol}>
            ...
            <Text
              variant="labelSmall"
              style={[styles.comparisonDelta, { color: deltaColor(resistsDelta) }]}
            >
              {resistsDelta === 0 ? '— same' : `${deltaStr(resistsDelta)} vs last week`}
            </Text>
          </View>
        </View>
      </Card.Content>
...
export default function ProgressScreen(): React.ReactElement {
  ...
  const [lastWeekStats, setLastWeekStats] = useState<WeeklyStats>(zeroWeekStats);
  ...
  const loadMonthData = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const lastDay = format(new Date(viewYear, viewMonth, daysInMonth), 'yyyy-MM-dd');
  ...
  const loadWeeklyStats = useCallback(async (): Promise<void> => {
    ...
    const lastWeekEvents = await getUrgeEventsInRange(db, lastWeekRange.start, lastWeekRange.end);
    ...
    const lastWeekResistCount = lastWeekEvents.filter((e) => e.outcome === 'success').length;
    const lastWeekPanicTotal = lastWeekEvents.filter((e) => e.outcome !== 'ongoing').length;
    const lastWeekPanicSuccess = lastWeekEvents.filter((e) => e.outcome === 'success').length;
    ...
    setLastWeekStats({
      ...
      totalDays: 7,
      panicSuccessRate: lastWeekPanicTotal > 0 ? lastWeekPanicSuccess / lastWeekPanicTotal : 0,
  ...
  const loadBestStreak = useCallback(async (): Promise<void> => {
    const twoYearsAgo = format(new Date(new Date().getFullYear() - 2, 0, 1), 'yyyy-MM-dd');
  ...
  const calendarDays = useMemo((): DayCell[] => {
    ...
    for (let i = 0; i < startDow; i++) {
      cells.push({ dateStr: '', dayNum: 0, isSuccess: false, isToday: false, inMonth: false });
    }
      ...
      <Card style={styles.card} mode="contained">
        <Card.Content>
          ...
          <View style={styles.legend}>
            ...
            <View style={styles.legendItem}>
              <View style={[styles.legendDot, { backgroundColor: colors.border, borderColor: colors.primary, borderWidth: 2 }]} />
      ...
      <Card style={styles.card} mode="contained">
        <Card.Content style={styles.statsContent}>
          ...
          <StatRow
            ...
            valueColor={weeklyStats.successDays > 0 ? colors.success : colors.muted}
          />
          <Divider style={styles.divider} />
          <StatRow
            ...
            valueColor={weeklyStats.panicSuccessCount > 0 ? colors.primary : colors.muted}
          />
          <Divider style={styles.divider} />
          <StatRow
            label="Resists this week"
            value={String(weeklyStats.resistCount)}
            valueColor={weeklyStats.resistCount > 0 ? colors.secondary : colors.muted}
          />
          <Divider style={styles.divider} />
          <StatRow
            label="Spend urges avoided"
            value={String(weeklyStats.spendAvoidedCount)}
            valueColor={weeklyStats.spendAvoidedCount > 0 ? colors.warning : colors.muted}
          />
        </Card.Content>
      </Card>
...
function StatRow({ label, value, valueColor }: StatRowProps): React.ReactElement {
  return (
