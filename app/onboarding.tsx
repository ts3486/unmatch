...
const GOAL_OPTIONS: GoalOption[] = [
  { id: 'reduce_swipe', label: 'Reduce swiping', description: 'Spend less time swiping through profiles.' },
  { id: 'reduce_open', label: 'Open apps less', description: 'Limit how often you open dating apps.' },
  { id: 'reduce_night_check', label: 'Stop late-night checking', description: 'Avoid checking apps before bed.' },
  { id: 'reduce_spend', label: 'Spend less', description: 'Reduce in-app spending and related costs.' },
];
...
export default function OnboardingScreen(): React.ReactElement {
  ...
  const [starterCourseEnabled, setStarterCourseEnabled] = useState<boolean>(true);
  ...
  const handleStart = useCallback(async (): Promise<void> => {
    ...
    try {
      const hasBudget = selectedGoal === 'reduce_spend' && budgetAmount.trim().length > 0;
      const parsedAmount = hasBudget ? Math.round(parseFloat(budgetAmount) * 100) : null;
        ...
        <View style={styles.centeredContent}>
          ...
          <View style={styles.welcomeTextBlock}>
            ...
            <Text variant="bodyLarge" style={styles.welcomeSubtitle}>
              This app helps you be intentional about dating apps — not remove them from your life, just put you back in control.
            </Text>
          </View>
        </View>
                  ...
                  <View style={styles.goalCardInner}>
                    <Text
                      ...
                      style={[
                        ...
                        selectedGoal === option.id && styles.goalCardTitleSelected,
                      ]}
                    >
                      {option.label}
                    </Text>
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Text variant="bodyMedium" style={styles.stepSubtitle}>
            A soft limit reminds you before you overspend. A pledge is a personal commitment.
          </Text>
          ...
          <SegmentedButtons
            value={budgetPeriod}
            onValueChange={(v) => { setBudgetPeriod(v as 'daily' | 'weekly'); }}
            buttons={[
          ...
          <SegmentedButtons
            value={budgetMode}
            onValueChange={(v) => { setBudgetMode(v as SpendingLimitMode); }}
            buttons={[
    ...
    <View style={styles.root}>
      ...
      >
        ...
        {(['stealth', 'normal', 'off'] as NotificationStyle[]).map((style_) => {
          const labels: Record<NotificationStyle, { title: string; desc: string }> = {
            stealth: { title: 'Stealth', desc: 'Generic-looking notifications, no app name visible.' },
            normal: { title: 'Normal', desc: 'Standard notifications with app context.' },
          ...
          return (
            ...
            >
              <Button
                mode="text"
                onPress={() => { setNotificationStyle(style_); }}
                style={styles.goalCardButton}
                contentStyle={styles.goalCardButtonContent}
                testID={`notif-${style_}`}
              >
        ...
        <View style={styles.courseToggleRow}>
          ...
          <Chip
            ...
            onPress={() => { setStarterCourseEnabled((prev) => !prev); }}
            style={[
              styles.toggleChip,
              starterCourseEnabled && styles.toggleChipSelected,
            ]}
            textStyle={starterCourseEnabled ? styles.triggerChipTextSelected : styles.triggerChipText}
          >
