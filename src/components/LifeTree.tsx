 ...
 * resist is earned.
 */
export function LifeTree({ level, resistCount }: LifeTreeProps): React.ReactElement {
  ...
  const rings = Array.from({ length: LIFE_TREE_RESISTS_PER_LEVEL }, (_, i) => i);
  ...
  const ringScales = useRef<Animated.Value[]>(
    Array.from({ length: LIFE_TREE_RESISTS_PER_LEVEL }, () => new Animated.Value(1)),
  ).current;
  ...
  useEffect(() => {
    if (resistCount > prevResistCount.current) {
      ...
      if (newlyFilledIndex >= 0 && newlyFilledIndex < LIFE_TREE_RESISTS_PER_LEVEL) {
        ...
        <View style={styles.headerRow}>
          <View>
            ...
            <Text variant="bodySmall" style={styles.sublabel}>
              {isMaxLevel ? 'Maximum level reached' : `Level ${level} of ${LIFE_TREE_CAP}`}
            </Text>
          </View>
        ...
        <View style={styles.progressSection}>
          ...
          {!isMaxLevel && (
            <Text variant="labelSmall" style={styles.progressLabel}>
              {LIFE_TREE_RESISTS_PER_LEVEL - resistsIntoCurrentLevel} more to Level{' '}
              {level + 1}
            </Text>
          )}
          {isMaxLevel && (
