...
import { RESIST_RANK_CAP, RESIST_RANK_RESISTS_PER_LEVEL } from '@/src/constants/config';
 ...
 * resist is earned.
 */
export function ResistRank({ level, resistCount }: ResistRankProps): React.ReactElement {
  ...
  const rings = Array.from({ length: RESIST_RANK_RESISTS_PER_LEVEL }, (_, i) => i);
  ...
  const ringScales = useRef<Animated.Value[]>(
    Array.from({ length: RESIST_RANK_RESISTS_PER_LEVEL }, () => new Animated.Value(1)),
  ).current;
  ...
  useEffect(() => {
    if (resistCount > prevResistCount.current) {
      ...
      if (newlyFilledIndex >= 0 && newlyFilledIndex < RESIST_RANK_RESISTS_PER_LEVEL) {
        ...
        <View style={styles.headerRow}>
          <View>
            ...
            <Text variant="bodySmall" style={styles.sublabel}>
              {isMaxLevel ? 'Maximum rank reached' : `Rank ${level} of ${RESIST_RANK_CAP}`}
            </Text>
          </View>
          ...
          <View style={styles.ringsContainer}>
            {rings.map((i) => (
              <Animated.View
                ...
                style={[
                  styles.ring,
                  i < resistsIntoCurrentLevel ? styles.ringFilled : styles.ringEmpty,
                  { transform: [{ scale: ringScales[i] ?? new Animated.Value(1) }] },
                ]}
              />
            ))}
        ...
        <View style={styles.progressSection}>
          ...
          {!isMaxLevel && (
            <Text variant="labelSmall" style={styles.progressLabel}>
              {RESIST_RANK_RESISTS_PER_LEVEL - resistsIntoCurrentLevel} more to Rank{' '}
              {level + 1}
            </Text>
          )}
          {isMaxLevel && (
